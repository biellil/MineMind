...
**Phase 9 (Placement + Crafting/Smelting Grounded) completa 2026-06-21:** primitivo `placeBlock` robusto implementado UMA vez (`placeBlockSafe`/`getRefAndFace`) — outcome deriva de `bot.blockAt` e engole o timeout do `blockUpdate` como falso-negativo; `shelter` e `station` consomem o mesmo wrapper (zero `bot.placeBlock` cru). Cadeia de crafting grounded: `ensureStation` (findBlock→navigate→place→POI `'station'`), `craft(item,count)` com gate de mesa antes do `bot.craft` (mata o throw "Recipe requires craftingTable"), `smelt` por item com `close()` em `finally` e verdade via `furnace.outputItem()`; `equip` como verbo de 1ª classe grounded por estado LOCAL (`heldItem`/slots, nunca delta) + pré-flight `selectToolFor` binário em dig/attack (sem ranking por tier — isso é Phase 10). 4 skills (placeBlock/craft/smelt/equip) registradas no `skillRegistry`+`toolRegistry` (11 cada). **G-01 fechado (09-05):** os 4 verbos agora são alcançáveis a partir da decisão do agente — `ActionDecisionSchema` estendido (mantido FECHADO, LLM-02/D-10) e os verbos fiados ao dispatch do execute node (estado agregado `'building'`, montagem de params físicos do target de alto nível, grounding/memória reusados). BUILD-01 + CRAFT-01/02/03/04 fechados no nível **comportamental**; 14/14 must-haves verificados (13 capacidade + G-01); suite global 432 pass / 0 fail / 1 skip; `tsc` exit 0. `placeRetries` (body do loop) deferido (D-04, idempotência presente, retry condicionado a teste ao vivo).

**Phase 10.1 (Paralelismo no processamento do LLM — deliberação concorrente) completa 2026-06-22:** removido o lock single-flight `inFlight` que serializava toda deliberação; substituído por `Semaphore` global (`= provider.maxConcurrency`) + gate-por-tipo (ação/reflexão/jogador) zero-dep (`concurrency.ts`); `maxConcurrency` exposto na interface `LlmProvider` e `AbortSignal` propagado por `decide`/`chat`; TOCTOU do `withSpendCap` fechado via `reserveCall`/`releaseCall` atômicos. No loop: `handleConversation` roteado pelo mesmo gate (`routePlayerTurn`), commit síncrono **merge-by-id** protege `holder.goals` do clobber concorrente, e preempção jogador→ação via `AbortController` com a reflexão sempre protegida (`pickDispatch` deixou de ser XOR). 15/15 must-haves verificados em código; suíte 477 pass / 1 skip / 1 fail (config `.env` pré-existente). Coexistência real ação×reflexão fica como HUMAN-UAT (requer LM Studio `Max Concurrent Predictions ≥ 2`). Destrava a concorrência autônomo+assistente da Phase 11.

**Phase 11.1 (Percepção espacial no contexto user/human) completa 2026-06-22:** `serializeContext` (`src/llm/prompts.ts`) deixou de descartar os `examples`/coordenadas que o snapshot já captura — agora alimenta o LLM com dado espacial CRU no formato híbrido `próx(x,y,z) Nm Δy±k` (posição absoluta + distância 3D + Δy com sinal explícito, `+0` nunca `-0`) para blocos próximos, mobs/entidades e jogadores. `BLOCK_COORD_BUDGET=18` virou o ÚNICO gate de quantas coords são emitidas, distribuídas por prioridade (troncos>pedra>minério>lixo); o `.slice(0,8)` de tipos foi removido, então todo tipo preserva `name×count`. Jogador com `position: null` degrada para `sem-pos` sem lançar. Invariantes preservados: mensagem user/human (nunca assistant), assinatura de `serializeContext` e call-sites inalterados, `types.ts`/`snapshot.ts` intocados, zero veredito textual de alcançabilidade (só Δy cru — o LLM infere). 6/6 must-haves verificados; suíte 504 pass / 1 skip / 1 fail (config `.env` pré-existente); `tsc` exit 0. Endereça uma causa-raiz do "bot parado" (decidir sem noção espacial); confirmação ao vivo de que o LLM local infere perto/longe a partir do Δy fica como verificação humana não-bloqueante.
...
---
*Last updated: 2026-06-22 — Phase 11.1 (Percepção espacial no contexto user/human) completa: `serializeContext` enriquecido com `próx(x,y,z) Nm Δy±k` para blocos/mobs/jogadores, teto global de 18 coords priorizado como único gate, `sem-pos` null-safe; 6/6 must-haves, mensagem user/human e assinatura intactas. — anterior: Phase 10.1 (Paralelismo no processamento do LLM — deliberação concorrente) completa: lock single-flight `inFlight` trocado por semáforo+gate-por-tipo, preempção jogador→ação, merge-by-id contra clobber; 15/15 must-haves em código, coexistência ação×reflexão ao vivo como HUMAN-UAT. — anterior: Phase 9 (Placement + Crafting/Smelting Grounded) completa incl. gap G-01 (09-05): `placeBlock` robusto implementado uma vez (shelter/station compartilham), cadeia craft/smelt/equip grounded por inventário/estado, 4 skills registradas E alcançáveis a partir da decisão do agente (enum LLM + dispatch) — BUILD-01 + CRAFT-01..04 fechados no nível comportamental, 14/14 must-haves verificados*
