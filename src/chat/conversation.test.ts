...
import { shouldRespond, handleConversation, SUPPORTED_REQUEST_KINDS } from './conversation'
...
test('handleConversation: ASSISTANT + pedido NÃO suportado => sem objetivo (nunca inválido)', async () => {
  ...
  await handleConversation(provider, holder, bot as never, 'Steve', 'constrói um castelo épico', 0)
...
test('SUPPORTED_REQUEST_KINDS é o conjunto fechado gather/follow/navigate (OQ3)', () => {
  expect([...SUPPORTED_REQUEST_KINDS].sort()).toEqual(['follow', 'gather', 'navigate'])
})
