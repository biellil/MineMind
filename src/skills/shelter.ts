...
//
// placeBlock é MÍNIMO aqui (1 chamada, try/catch) — robustez definitiva = Fase 9. Guarda
// anti-lava/caverna: checa blockAt 2 abaixo ANTES de cavar (não cair em lava nem em vazio).
//
...
export async function shelter(bot: Bot, rawParams: unknown): Promise<SkillResult> {
  ...
  if (canDig) {
    // Branch CAVAR-E-TAMPAR: cava o bloco 1 abaixo, desce e tampa o topo (placeBlock mínimo).
    try {
      const below1 = bot.blockAt(pos.offset(0, -1, 0))
      if (below1 && !UNSAFE_BELOW.has(below1.name)) {
        await bot.dig(below1)
      }
      // Tampa o topo: coloca bloco usando o bloco da cabeça como referência (face para baixo).
    ...
    // Branch PILAR 1×1: equipa o bloco, pula e coloca sob os pés no ápice (placeBlock mínimo).
    try {
      ...
      const belowRef = bot.blockAt(pos.offset(0, -1, 0))
      if (belowRef) {
        await bot.placeBlock(belowRef, makeVec(0, 1, 0))
      }
    } catch (err) {
      threw = err
    } finally {
